...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/requests/:id/accept", async (req: Request, res: Response) => {
    ...
    await db.insert(notifications).values({
      ...
      body: `${me.name} accepted your request for ${updated.position} at ${updated.targetCompany}. They have 24 hours to refer you.`,
      linkUrl: "/referrals",
    }).catch(() => {});
  ...
  app.post("/api/recommendations", async (req: Request, res: Response) => {
    ...
    if (!r || !r.acceptedById) return res.status(403).json({ error: "Can only recommend after a completed referral" });
    const [me] = await db.select().from(users).where(eq(users.id, userId)).limit(1);
  ...
  app.get("/api/recommendations/:userId", async (req: Request, res: Response) => {
    const recs = await db.select().from(recommendations)
      .where(eq(recommendations.forId, req.params.userId))
  ...
  app.get("/api/chat/:requestId", async (req: Request, res: Response) => {
    const msgs = await db.select().from(chatMessages)
      .where(eq(chatMessages.requestId, req.params.requestId))
